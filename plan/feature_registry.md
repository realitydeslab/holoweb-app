...
| E6 | hand-tracking (Vision + LiDAR, 25 joints, pinch select, grab squeeze, One Euro); handedness from geometry (back-of-hand cross product), Vision chirality only as `visionChirality` | device via screen fixtures: palm photo 187 hands 21/21 depth, 23.4/s; first-person two-hand photo (hands2): 178/178 results left-of-screen = "left", 22.2/s, Vision 18–20 ms | R hands.*, hands.chirality-first-person, xr.required-hand-mesh; human (real hands) |
| E7 | image-tracking ([explainer](https://github.com/immersive-web/image-tracking/blob/main/explainer.md)): trackedImages, scores, results, imageSpace convention (`anchor * Rx(-90°)`) | native device (image-check vs marker on the laptop screen: 280 tracked results, width 0.150 m, axes orthonormal, z·toCamera 0.79, y·up 0.86); polyfill headless in Chromium + WebKit (U image-tracking, image-snapshot; E example + fixture, PlayCanvas, Needle); device: snapshot fix pending recheck (was untrackable) | R image.scores, image.arkit-detection-images, image.results-shape, image.marker-tracked, image.width, image.axes-orthonormal, image.z-toward-viewer, image.y-up-when-upright, *-image-tracking.*; human (marker in view) |
