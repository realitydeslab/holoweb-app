# holoweb.app server files

Files in this directory are deployed to the `holoweb.app` web server. They are not part of the iOS build.
...
```
https://holoweb.app/.well-known/apple-app-site-association
```
...
Apple fetches the file through its CDN (`https://app-site-association.cdn-apple.com/a/v1/holoweb.app`), so changes can take up to a day to reach devices. For development builds, append `?mode=developer` to the associated domain entry, or enable Settings > Developer > Associated Domains Development on the device.

What the file declares:

- `applinks`: the full app (`KR9H35SQQ9.org.realitydeslab.holoweb`) opens links matching `/c*`, for example `https://holoweb.app/c?url=https%3A%2F%2Fholoweb.app%2Ftest2%2F`.
- `appclips`: the App Clip (`KR9H35SQQ9.org.realitydeslab.holoweb.Clip`) may be invoked from `holoweb.app` URLs.

The matching entitlements are in `HoloWeb/HoloWeb.entitlements` (`applinks:holoweb.app`, `appclips:holoweb.app`) and `HoloWeb/HoloWebClip.entitlements` (`appclips:holoweb.app`).

Check the deployed file:

```sh
curl -sI https://holoweb.app/.well-known/apple-app-site-association | grep -i content-type
curl -s https://app-site-association.cdn-apple.com/a/v1/holoweb.app
```

## Safari smart banner

Pages on `holoweb.app` that should offer the App Clip include this tag in `<head>`:

```html
<meta name="apple-itunes-app" content="app-clip-bundle-id=org.realitydeslab.holoweb.Clip, app-id=APP_STORE_ID">
```

Replace `APP_STORE_ID` with the numeric Apple ID of the full app from App Store Connect (App Information > Apple ID). The banner opens the App Clip card for the current page URL, so the page's own URL should be a `https://holoweb.app/c?url=...` link, or the banner should be placed on pages served under `/c`.
